...
// convention actually governs (the invite contact line), not a whole-document text
// scan — section kickers (e.g. "Identity / DEMO") and #statusText are a persistent
// "demo build" watermark, a different, intentionally-excluded category (see the
// convention comment), so a blanket scan would false-positive on them, on the
// convention comment's own explanatory text, and on ordinary prose sentences.
// Witness citations were removed from scope: the witness chapter intentionally carries
// quotes without source attribution, so there is no citation field to audit.
