...
- **Placeholder convention**: the fields governed by this convention (the invite contact line) consistently read `DEMO / 待确认`. Witness quotes in the “别人眼中的我” chapter intentionally carry no source attribution — only the words, no named roles. Section kickers (e.g. "Identity / DEMO") and the floating `#statusText` remain a separate, intentional "demo build" watermark, not a content placeholder governed by this convention — see the convention comment at the top of `<body>` in `index.html`.

## V8.1 refinement addendum (2026-08-26)

This is an addendum, not a revision — the 11-section narrative, DOM semantics, interaction grammar (single-select / `aria-pressed`) and the 12-token palette are unchanged. It records the V8.1 readability / motion / trajectory-crossing / atmosphere pass.

- **Typography reinforcement**: all label-type small text (markers, indexes, stamps, orbit nodes, summaries, contact line, anchored scene text active state) now sits at ≥12px / weight 500; anchored text active state is 13px/500/opacity 1. Long-form body copy (14px expanded notes) keeps 400 as editorial reading weight. No new color value introduced.
- **Entrance motion + `motion-ready` transaction**: chapter title chains (kicker → title → lead → stamp → poem), per-line poem fade and mantra parallax now run inside one initialization transaction — DOM defaults to fully visible, all ScrollTriggers register first, and `html.motion-ready` is committed only as the final step after total success; any throw reverts the context and leaves the page in its default visible state. The reading-progress rail + 35% counter is a separate, independently-committed concern (`initProgress`), because reading position is function, not decoration. Every opacity/transform property has exactly one writer (section fade / list waterfall / title chain / anchor layer selectors are mutually exclusive via `:not()` chains).
- **Visible text-crossing treatment (geometry unchanged)**: trajectory and ridge geometry is untouched; instead body text (`.field-title/.field-lead/.thought-thread`) carries a same-background halo (`text-shadow: 0 0 6px var(--bg), 0 0 14px var(--bg)`) and the trajectory opacity ceiling drops 0.5 → 0.38. Lines still intersect text geometrically but are pixel-occluded at glyph locations. The acceptance detector skips sample points inside the halo radius; `.section-mantra` watermarks stay intentional. Measured: 42 visible hits → 0.
- **Chapter atmosphere images**: three editorial images (misty ridge / seedling roots / mountain flowers, generated to the locked dark-green palette, edge-faded, no people, no text) sit behind journey / beliefs / invite content as `<img class="chapter-plate" loading="lazy" decoding="async" alt="">` — real lazy loading, absolutely positioned (`inset: 0`, z between scene and content, never negative z-index), opacity 0.3 (0.18 below 840px), feathered via `mask-image` + `-webkit-mask-image` with a low-opacity full-image fallback when masks are unsupported. Image failure never affects body readability.
- **Asset split**: WebP runtimes (≤1920px wide, 26–63KB) live in `assets/images/` and are the only images entering the build; generation source PNGs are archived in `artifacts/source/` and excluded from the build.
- **Verification**: full re-audit + 4 regression specials (init-throw visibility, cold-start image requests, fast-scroll residue, broken-image readability) all passed; see `artifacts/visual-test-report.md` round 3 and `after-*.png` screenshots.
